...
export const deleteSnippet =mutation({
  ...
  handler: async(ctx,args) =>{
    ...
    if(snippet.userId !== identity.subject){
      throw new Error("Not authroized to delete this snippet!")
    }
...
export const starSnippet = mutation({
  ...
  handler:async(ctx,args)=>{
    ...
    if(!identity) throw new Error("Not authenticated!")
      ...
      const existing = await ctx.db.query("stars").withIndex("by_user_id_and_snippet_id").filter((q)=>q.eq(q.field("userId"),identity.subject) && q.eq(q.field("snippetId"),args.snippetId)).first()
